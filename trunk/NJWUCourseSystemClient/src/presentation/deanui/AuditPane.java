...
public class AuditPane extends JPanel{
	...
	public void showAuditTable(String type,String facultyName,String grade) {
    	AuditUI audit=new AuditUI();
    	if(type.equals(AuditType.准入.toString())){
    		audit.getAllowInTable(auditTable, facultyName, grade);
    	}else if(type.equals(AuditType.准出.toString())){
    		audit.getAllowOutTable(auditTable, facultyName, grade);
    	}else{
    		audit.getGraduateTable(auditTable, facultyName, grade);
    	}
	}
