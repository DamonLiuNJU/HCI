...
import presentation.teacherui.ID_NameComparisonFrame;
...
public class CourseGUI {
	...
	void initialPublishPanel() {
		...
		JButton searchTeacherBut=new JButton("本院教师查询");
		searchTeacherBut.setBorderPainted(false);	
		searchTeacherBut.addActionListener(new ActionListener(){

			@Override
			public void actionPerformed(ActionEvent e) {
				// TODO Auto-generated method stub
				new  ID_NameComparisonFrame(id);
			}
			
		});
		searchTeacherBut.setBounds(frameWidth*19/30 , frameHeight /29,
				frameWidth /10, frameHeight /30);
	
		publishPanel.add(searchTeacherBut);
	...
	void initialRevisePanel( String courseID) {
		...
		revisePanel.add(searchTeacherBut);
		revisePanel.add(staticBut);
		...
		add.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (testInfo()) {
					...
					if(info.getModule().equals("必修课")){
						new Course().addCourseforStudent(id,info.getCno());
					}
	...
	if (courseID.equals("")){
		...
		if(teacherID.equals("")){
			...
			boolean if_exist=new Course().testTeacher(id,teacherID);//teacher.isTeacherUsable(id,teacherID)
			if(!if_exist){
