...
//				valid = s.current();//接口修改
				...
				if (valid) {
					final String modelname = "通识课";
					SelectCourseModule scp = new SelectCourseModule();
					scp.getSelectCommonKnowlegeFrame(modelname, student_id);
				} else {
...
//					new SelectCourseModule().getSelectCommonKnowlegeFrame(modelname, student_id);
	...
	//这个可以单独出来一个类
	private JFrame getSelectCommonKnowlegeFrame(final String modelname,final String studentid) {

		 
		Vector<Vector<String>> mycourselist =new SelectCourseInfo().getSelectedCourse(studentid, modelname); //store the lessons he has chosen;
...
//		Vector<Vector<String>> content = new Vector<Vector<String>>();
		...
		commitselect.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				CourseSelectionVO sv = new CourseSelectionVO(studentid);
				for (int i = 0; i < tempselect.getRowCount(); i++) {
					String course_id = null;
					course_id = (String) tempselect.getValueAt(i, 0);
					boolean courseexists = false;
					courseexists = new SelectCourseInfo().courseExist(studentid, course_id);
					if(courseexists){
...
//		Tool.setIcon(Tool.add, addtotempselect);
		...
		cancelselect.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				si.removeCourse(studentid,course_id);
